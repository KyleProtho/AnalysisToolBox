...
# Declare function
CreateClassificationTree = function(dataframe,
                                    outcome_variable,
  ...
  if (use_bootstrap_aggregating) {
    ...
    classification_tree_model = train(
      formula_string,
      data = data_train,
      method = "treebag",
      trControl = ctrl,
    ...
    if (show_important_predictor_plot) {
      V = varImp(classification_tree_model)[["importance"]]
      p = ggplot(V, 
    ...
    # Create regression tree model
    classification_tree_model = rpart(
  ...
  return_dictionary = list(classification_tree_model,
                           data_train,
                           data_test)
