...
helper_t_test <- function(data,
  ...
  if (test_normality_assumption) {
    ...
    title_for_plot = paste("Histogram - Examining the Normality Assumption")
  ...
  if (test_homogeneity_of_variances_assumption) {
    ...
    p = ggplot(data,
      ...
      geom_dotplot(binaxis = 'y', 
                   stackdir = 'center', 
                   dotsize = .5) +
      labs(title = "Box-Beeswarm Plot - Examining the Homogeneity of Variances Assumption",
           caption = caption_string,
           x = "Groups",
           y = outcome_column) + 
      theme_hc() + 
...
t_test_results_am = helper_t_test(data = df_mtcars,
                                  ...
                                  test_homogeneity_of_variances_assumption = TRUE,
                                  plot_distribution_of_means = TRUE,
                                  plot_difference_in_means = TRUE)
